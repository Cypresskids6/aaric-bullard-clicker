...
let vSound2 = new Audio("./sounds/vilagerSound2.mp3");
let vSound3 = new Audio("./sounds/vilagerSound3.mp3");
...
if(blockAmount>= dimensionArray[currentDimension+1]){
    ...
    if ((currentDimension===0) && (blockUpgradeCounter>=15) && (pickaxeUpgradeCounter>=14)){
        updateDimension();
...
if(blockAmount>efficiencyList[enchantTracker[1]]){
    ...
    enchantTracker[1]++;
    efficiencyLabel.innerHTML = efficiencyList[enchantTracker[1]];
    autoclickDuration-=100;
    if(!blacksmithID===null){
        clearInterval(blacksmithID);
        statAutoClicker("Blacksmith",autoclickDuration)
    }
    if(!farmerID===null){
        clearInterval(farmerID);
        statAutoClicker("Famer",autoclickDuration)
     }
     if(!librarianID===null){
        clearInterval(librarianID);
        statAutoClicker("librarian",autoclickDuration)
   }
}
});
...
villager.object.addEventListener("mouseleave",()=>{
  ...
  villager.object.addEventListener("click",()=>{
    ...
    if(blockAmount>=villager.cost){
       statAutoClicker(villager.name,autoclickDuration);
    blockAmount = blockAmount - villager.cost;
    blockAmountLabel.innerHTML = "Blocks: " + blockAmount;
    villager.level++;
    let update = villagerLevelList[villager.id];
    villager.cost = update[villager.level];
    textBox.innerHTML = textBox.innerHTML = `${villager.name} upgrade: ${villager.cost}`; 
    }
    
  });
});

function statAutoClicker(name){
    switch(name){
        case "Blacksmith":
